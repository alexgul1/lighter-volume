---
url: https://docs.nado.xyz/liquidations
title: Liquidations
---
...
When a subaccount's maintenance health falls below zero, the account becomeseligiblefor liquidation. Nado doesnotautomatically liquidate it; a liquidator must submit a transaction to kick off the flow. Any user can do this by calling theliquidate-subaccountAPI endpointor the on-chain contract method to buy discounted assets or repay marked-up liabilities, earning the spread while restoring solvency. Incentives keep liquidators active, so liquidations are typically picked up quickly.

[liquidate-subaccountAPI endpoint](/developer-resources/api/gateway/executes/liquidate-subaccount)
...
TheNado Liquidity Provider (NLP)vault actively participates in liquidations to maintain platform stability and generate yield for liquidity providers. When a liquidatable position is detected, the NLP may front-run external liquidators under certain conditions:

[Nado Liquidity Provider (NLP)](/nlp)
...
- Calculate penalty:penalty = (1 - perp_maint_asset_weight) / 10

Calculate penalty:penalty = (1 - perp_maint_asset_weight) / 10

`penalty = (1 - perp_maint_asset_weight) / 10`
- Apply minimum floor:penalty = max(penalty, 0.0025)(0.25%)

Apply minimum floor:penalty = max(penalty, 0.0025)(0.25%)

`penalty = max(penalty, 0.0025)`
- Liquidation price (discount):spot_price × (1 - penalty)

Liquidation price (discount):spot_price × (1 - penalty)

`spot_price × (1 - penalty)`

Buying Spread (amount < 0): Buying spot to cover short, closing perp long

- Calculate penalty:penalty = (spot_maint_liability_weight - 1) / 10

Calculate penalty:penalty = (spot_maint_liability_weight - 1) / 10

`penalty = (spot_maint_liability_weight - 1) / 10`
- Apply minimum floor:penalty = max(penalty, 0.0025)(0.25%)

Apply minimum floor:penalty = max(penalty, 0.0025)(0.25%)

`penalty = max(penalty, 0.0025)`
- Liquidation price (markup):spot_price × (1 + penalty)
...
- Positive Outcome: The subaccount receives USDT0 (e.g., from selling positive spot balances or closing profitable perps).

Positive Outcome: The subaccount receives USDT0 (e.g., from selling positive spot balances or closing profitable perps).

- Negative Outcome: The subaccount spends USDT0 (e.g., repaying borrows or covering losing perps).
...
Suppose you have 31,000 USDT0 and short 10 ETH spot ($30,000 at $3000 / ETH) now. ETH spot maintenance_liability_weight is 1.05, then:
...
But since 0.2% < 0.5% minimum, the0.5% floor applies:
...
- Perpetual Socialization: Pro-rata deductions from other positions in the same market (e.g., all ETH-PERP holders share a 0.1% collateral trim).

Perpetual Socialization: Pro-rata deductions from other positions in the same market (e.g., all ETH-PERP holders share a 0.1% collateral trim).

- USDT0 Depositor Socialization: Remaining shortfalls hit USDT0 balances across the platform.
...
[PreviousSubaccounts & Health](/subaccounts-and-health)
[NextPnL Settlements](/pnl-settlements)
