---
url: https://nadohq.github.io/nado-python-sdk/_modules/nado_protocol/client/apis/rewards/execute.html
---

# Source code for nado_protocol.client.apis.rewards.execute
...
[docs]class RewardsExecuteAPI(NadoBaseAPI):
...
[docs]    def stake(self, amount: int, signer: Optional[LocalAccount] = None) -> str:
        signer = self._get_signer(signer)
        return self.context.contracts.stake(amount, signer)

[docs]    def unstake(self, amount: int, signer: Optional[LocalAccount] = None) -> str:
        signer = self._get_signer(signer)
        return self.context.contracts.unstake(amount, signer)

[docs]    def withdraw_unstaked(self, signer: Optional[LocalAccount] = None):
        signer = self._get_signer(signer)
        return self.context.contracts.withdraw_unstaked(signer)

[docs]    def claim_usdc_rewards(self, signer: Optional[LocalAccount] = None):
        signer = self._get_signer(signer)
        return self.context.contracts.claim_usdc_rewards(signer)

[docs]    def claim_and_stake_usdc_rewards(self, signer: Optional[LocalAccount] = None):
        signer = self._get_signer(signer)
        return self.context.contracts.claim_and_stake_usdc_rewards(signer)

[docs]    def claim_foundation_rewards(self, signer: Optional[LocalAccount] = None):
        """
