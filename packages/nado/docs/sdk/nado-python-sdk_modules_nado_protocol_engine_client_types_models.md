---
url: https://nadohq.github.io/nado-python-sdk/_modules/nado_protocol/engine_client/types/models.html
---

# Source code for nado_protocol.engine_client.types.models
...
[docs]class ResponseStatus(StrEnum):
    SUCCESS = "success"
    FAILURE = "failure"

[docs]class EngineStatus(StrEnum):
    ACTIVE = "active"
    FAILED = "failed"

[docs]class ApplyDelta(NadoBaseModel):
    product_id: int
    subaccount: str
    amount_delta: str
    v_quote_delta: str

[docs]class ApplyDeltaTx(NadoBaseModel):
    apply_delta: ApplyDelta

[docs]class SubaccountHealth(NadoBaseModel):
    assets: str
    liabilities: str
    health: str

[docs]class SpotBalance(NadoBaseModel):
    amount: str

[docs]class SpotProductBalance(NadoBaseModel):
    product_id: int
    balance: SpotBalance

[docs]class PerpBalance(NadoBaseModel):
    amount: str
    v_quote_balance: str
    last_cumulative_funding_x18: str

[docs]class PerpProductBalance(NadoBaseModel):
    product_id: int
    balance: PerpBalance

[docs]class ProductRisk(NadoBaseModel):
...
[docs]class ProductBookInfo(NadoBaseModel):
    size_increment: str
    price_increment_x18: str
    min_size: str
    collected_fees: str

[docs]class BaseProduct(NadoBaseModel):
    product_id: int
    oracle_price_x18: str
    risk: ProductRisk
    book_info: ProductBookInfo

[docs]class SpotProductConfig(NadoBaseModel):
    token: str
...
[docs]class SpotProductState(NadoBaseModel):
...
[docs]class Asset(NadoBaseModel):
    product_id: int
...
[docs]class MarketPair(NadoBaseModel):
    ticker_id: str
    base: str
    quote: str

[docs]class SpotApr(NadoBaseModel):
    name: str
    symbol: str
    product_id: int
    deposit_apr: float
...
[docs]class Orderbook(NadoBaseModel):
    ticker_id: str
    timestamp: int
